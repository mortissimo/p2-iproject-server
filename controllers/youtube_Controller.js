...
class YoutubeController {
    static fetchSearchedMovie(req, res, next){
        ...
        .then(({data}) =>{
            const payload = data.items.map(data =>{
                return {videoId:data.id.videoId, title:data.snippet.title, channelTitle:data.snippet.channelTitle, url: data.snippet.thumbnails.medium.url, description: data.snippet.description, publishTime: data.snippet.publishTime};  
            })
    ...
    static fetchPopularVideos(req, res, next){
        ...
        axios({
            method: "GET",
            url : `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&chart=mostPopular&key=AIzaSyDxp4PnAEHjyffwcSkhL4-xTqY-kFfA0vc&maxResults=18`   
        })
        .then(({data}) =>{
            // const payload = data.items.map(data =>{
